...
public class animalGuesserShell
{
   ...
   public static void main(String argv[])throws IOException
   {
      ...
      String[] questionList = readFile("animal.txt");
         
      while(leftChild < questionList.length - 1 && !questionList[leftChild].equals("null"))
      {
         System.out.println(questionList[parent]);
         String response = input.nextLine();
         if(isYes(response) != true)
         {
            parent = leftChild;
            status = 1;
         }
      ...
      else
      {
         ...
         System.out.println("Enter a question that will help identify this animal: ");
         String question = input.nextLine();
         
         String temp = questionList[parent];
         questionList[parent] = question;
         if(parent * 2 > questionList.length)
         {
            String[] tempList = new String[questionList.length * 2];
            for(int i = 0; i < questionList.length; i++)
            {
               tempList[i] = questionList[i];
            }
            questionList = tempList;
         }
         questionList[(parent * 2)] = temp;
         questionList[(parent * 2) + 1] = "Is your animal a/an " + animal;
         String yesOrNo = input.nextLine();
         if(yesOrNo.toLowerCase().equals("yes"))
            System.out.println("Got it.");
         writeToFile(questionList, "animal.txt");
      }     
   }
}