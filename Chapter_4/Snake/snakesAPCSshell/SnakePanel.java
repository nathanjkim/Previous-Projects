...
//post:  respawn a pellet in a clear location (called when a power pellet or split powerup is eaten, and restart/constructor)
   public void spawnPellet()
   {
      ...
      pelletR = (int)(Math.random()*(numRows-1));					//temporary assignment to keep things compiling
      pelletC = (int)(Math.random()*(numRows-1));				//temporary assignment to keep things compiling
      boolean wallIsThere = false;
      if(board[pelletR][pelletC] == 1)	//looks to see if there is a wall in the picked spawn location
         wallIsThere = true;
      while(wallIsThere == true)
      {
         pelletR = (int)(Math.random()*(numRows-1));					
         pelletC = (int)(Math.random()*(numRows-1));
         wallIsThere = false;
         if(board[pelletR][pelletC] == 1)	//looks to see if there is a wall in the picked spawn location
            wallIsThere = true;
      ...
      if(powerupSpawn)
      {
       //**********************************FIX 1b*****************************************************/
         ...
         powerupR = (int)(Math.random()*(numRows-1));					//temporary assignment to keep things compiling
         powerupC = (int)(Math.random()*(numRows-1));				//temporary assignment to keep things compiling
         wallIsThere = false;
         if(board[powerupR][powerupC] == 1)	//looks to see if there is a wall in the picked spawn location
            wallIsThere = true;
         while(wallIsThere == true)
         {
            powerupR = (int)(Math.random()*(numRows-1));					
            powerupC = (int)(Math.random()*(numRows-1));
            wallIsThere = false;
            if(board[powerupR][powerupC] == 1)	//looks to see if there is a wall in the picked spawn location
               wallIsThere = true;
